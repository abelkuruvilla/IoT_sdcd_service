...
mySQLdb = "iotcache"
deviceId = "5ab38259437af5970fca35e5"
...
def get_decrypt_key_from_server(topic, device_id):
    ...
    response = requests.get("http://"+broker_address +
                            ":"+str(webPort)+"/getktm", params=parameter)
    data = response.json()
    return data
...
def save_to_cache(key_details):
    try:
            ...
            #print myDatetime
            cur.execute("INSERT INTO KTM (keyId,keyT,val) values('%s','%s','%s') ON DUPLICATE KEY UPDATE keyT='%s',val='%s' " % (
                key_details['_id'], key_details['keyT'], myDatetime, key_details['keyT'], myDatetime))
            db.commit()
    except:
            print 'error'
